...
{name:'海南', cityList:[{name:'海口市', areaList:['市辖区','秀英区','龙华区','琼山区','美兰区']},{name:'三亚市', areaList:['市辖区']},            
 ...
 */
$(function(){
	for (var int = 0; int < provinceList.length; int++) {//35个省
		var name = provinceList[int].name;
		$("#cmbProvince").append("<option value='"+name+"'>"+name+"</option>");//将35个省添加进去
	}
	var s = provinceList[0].cityList;
	for ( var i in s) {
		$("#cmbCity").append("<option value='"+s[i].name+"'>"+s[i].name+"</option>");//将第0下标的城市添加进去
		$("#cmbArea").append("<option value='"+s[i].areaList+"'>"+s[i].areaList+"</option>");//将第0下标的县添加进去
	}
	
	$("#cmbProvince").change(function(){
		$("#cmbCity").empty();//删除已有的元素
		var hn = $("#cmbProvince").val();//获取变动的值
		var intes = 0;//利用intes替换下标
		for (var int2 = 0; int2 < provinceList.length; int2++) {
			var g = provinceList[int2].name; //循环所有名字匹配
			if(g == hn){//当前的值等于循环的值捕捉下标
				intes = int2;
			} 
		}
		var obj = provinceList[intes].cityList;//根据下标获取他的对象,cityList(市级和县级)
		for ( var s in obj) {//循环这个对象
			$("#cmbCity").append("<option value='"+obj[s].name+"'>"+obj[s].name+"</option>");//遍历出来然后添加市级
			var b = $("#cmbCity :selected").val();//获得当前显示的城市
			if(b == obj[s].name){//当前显示的值等于遍历的值时
				$("#cmbArea").empty();//清空县级下拉框
				var Area = obj[s].areaList;//获得县级对象
				for ( var i in Area) {//继续遍历
					$("#cmbArea").append("<option value='"+Area[i]+"'>"+Area[i]+"</option>");//遍历出来然后添加县级
				}
			}
		}
	});
	
	$("#cmbCity").change(function(){//当省级值更换时执行
		var hn = $("#cmbProvince").val();//获取变动的值
		var intes = 0;//利用intes替换下标
		for (var int2 = 0; int2 < provinceList.length; int2++) {
			var g = provinceList[int2].name; //循环所有名字匹配
			if(g == hn){//当当前的值
				intes = int2;//当前的值等于循环的值捕捉下标
			} 
		}
		var obj = provinceList[intes].cityList;//根据下标获取他的对象,cityList(市级和县级)
		for (var s in obj) {//循环这个对象
			var name = obj[s].name;//获得遍历的城市
			var city = $("#cmbCity").val();//获得当前显示的城市
			if(name == city){//相等时候
				$("#cmbArea").empty();//清空县级
				var Area = obj[s].areaList;//获得县级对象
				for ( var i in Area) {//再次遍历
					$("#cmbArea").append("<option value='"+Area[i]+"'>"+Area[i]+"</option>");//遍历出来然后添加县级
				}
			}
		}
